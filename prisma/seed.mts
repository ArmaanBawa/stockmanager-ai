...
async function main() {
    ...
    const supplier2 = await prisma.supplier.create({
        data: {
            ...
            email: 'priya@globalchip.com',
            phone: '+91 98765 22222',
            address: '789 Electronics Zone, Bangalore',
            businessId: business.id,
        },
    });
    ...
    const supplier3 = await prisma.supplier.create({
        data: {
            ...
            email: 'sanjay@packright.com',
            phone: '+91 98765 33333',
            address: '56 Packaging Lane, Chennai',
            businessId: business.id,
        },
    });
    ...
    const product1 = await prisma.product.create({
        data: {
            name: 'Circuit Board PCB-X200',
            sku: 'PCB-X200',
            description: 'High-performance multi-layer circuit board',
            unitPrice: 850,
            unit: 'pcs',
            reorderLevel: 50,
            businessId: business.id,
    ...
    const product2 = await prisma.product.create({
        data: {
            name: 'ARM Cortex M4 Chip',
            sku: 'ARM-M4-001',
            description: '32-bit microcontroller chip',
            unitPrice: 320,
            unit: 'pcs',
            reorderLevel: 100,
            businessId: business.id,
    ...
    const product3 = await prisma.product.create({
        data: {
            name: 'LED Display Module 5"',
            sku: 'LED-DSP-5',
            description: '5-inch OLED display panel',
            unitPrice: 1200,
            unit: 'pcs',
            reorderLevel: 30,
            businessId: business.id,
    ...
    const product4 = await prisma.product.create({
        data: {
            name: 'Anti-Static Packaging Box',
            sku: 'PKG-AS-100',
            description: 'ESD-safe packaging for electronics',
            unitPrice: 45,
            unit: 'box',
            reorderLevel: 200,
            businessId: business.id,
    ...
    const product5 = await prisma.product.create({
        data: {
            name: 'Capacitor Set 100μF',
            sku: 'CAP-100UF',
            description: 'Electrolytic capacitor pack',
            unitPrice: 15,
            unit: 'pack',
            reorderLevel: 500,
            businessId: business.id,
    ...
    await prisma.inventoryLot.create({
        data: {
            lotNumber: 'LOT-DEMO-001', quantity: 50, remainingQty: 35, costPerUnit: 850,
            businessId: business.id, productId: product1.id, orderId: order1.id, receivedAt: new Date('2026-02-05'),
        },
    });
    ...
    await prisma.ledgerEntry.create({
        data: {
            type: 'PURCHASE', quantity: 50, unitPrice: 850, totalAmount: 42500,
            description: 'Purchase of Circuit Board PCB-X200 via order PO-DEMO-001',
            businessId: business.id, productId: product1.id, orderId: order1.id, supplierId: supplier1.id, createdAt: new Date('2026-02-05'),
        },
    });
    ...
    await prisma.order.create({
        data: {
            orderNumber: 'PO-DEMO-002', status: 'IN_MANUFACTURING', totalAmount: 64000,
            businessId: business.id, supplierId: supplier2.id,
    ...
    await prisma.order.create({
        data: {
            orderNumber: 'PO-DEMO-003', status: 'DISPATCHED', totalAmount: 36000,
            businessId: business.id, supplierId: supplier1.id,
    ...
    await prisma.order.create({
        data: {
            orderNumber: 'PO-DEMO-004', status: 'PLACED', totalAmount: 9000,
            businessId: business.id, supplierId: supplier3.id,
    ...
    console.log('📧 Login: rahul@acme.com / password123');
