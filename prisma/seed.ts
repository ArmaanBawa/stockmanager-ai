...
async function main() {
    ...
    // Create inventory lot for delivered order
    await prisma.inventoryLot.create({
        data: {
            ...
            orderId: order1.id,
            receivedAt: new Date('2026-02-05'),
        },
    });
    ...
    await prisma.inventoryUsage.create({
        data: {
            quantity: 15,
            reason: 'Used in production batch #45',
            businessId: business.id,
            productId: product1.id,
    ...
    await prisma.order.create({
        data: {
            ...
            statusHistory: {
                create: [
                    { status: 'PLACED', note: 'Packaging order placed', createdAt: new Date('2026-02-22') },
                ],
            },
        },
    });

    // Add more inventory for other products
    await prisma.inventoryLot.create({
        data: {
            lotNumber: 'LOT-DEMO-002',
            quantity: 300,
            remainingQty: 80,
            costPerUnit: 320,
            businessId: business.id,
